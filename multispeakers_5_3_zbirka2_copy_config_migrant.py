...
INP_MELODY_SUBFOLDER = "Inp_melodu"
...
def ensure_melodies_copied():
    ...
    if _temp_folder is None:
        logging.warning("\nensure_melodies_copied: _temp_folder не встановлено")
        return
    ...
    for src, dst in melody_map:
        try:
            if src and Path(src).exists():
                ...
                logging.warning(f" ensure_melodies_copied: Мелодія не знайдена: \n{src}")
        except Exception as e:
            logging.warning(f"\nНе вдалося скопіювати мелодію {src}: {e}")
...
def save_fragment_and_tts(fragment_l_text: str, voice_l_tag: str, chapter_l_folder_name: str, fragment_l_num: int) -> Tuple[bool, Optional[Path]]:
    ...
    if _current_text_folder is None or _current_audio_folder is None:
        logging.error("\nsave_fragment_and_tts: папки не ініціалізовані")
        return False, None
    ...
    if success:
        logging.info(f"\nФрагмент  озвучено: {audio_l_path} (голос: {voice_l_tag})")              
...
def save_fragment(text: str, voice_tag: str, chapter_folder: Path, frag_num: int, test_l_wav: Optional[Path]=None) -> Path:
    ...
    logging.info(f"\nЗбережено текст фрагменту: {txt_path}")
    ...
    if success:
        logging.info(f"Фрагмент озвучено: {audio_path}")
    _current_fragment_counter +=1
    return audio_path
...
def init_project_root(input_file: str, output_folder: str) -> Path:
    ...
    logging.info(f"\nІніціалізовано проєкт: {_project_root}")
    return project_root
...
def start_new_chapter(raw_chapter_line: str):
    ...
    # Перший фрагмент починається з назви (якщо є)
    if _current_chapter_name_for_fragment:
        _current_block_text.append(_current_chapter_name_for_fragment)
    logging.info(f"\nПочато нову главу: {_current_chapter_folder} (голос: {_current_voice_tag})")
...
def start_new_voice_block(line_with_tag: str):
    ...
    if m:
        ...
        logging.warning(f"\nstart_new_voice_block: не знайдено тегу в рядку: {line_with_tag}")
...
def finalize_chapter():
    ...
    if _current_chapter_name_for_files is None:
        logging.debug("\nfinalize_chapter: нема відкритої глави")
        return
...
def process_input_file(input_file: str):
    ...
    for raw_line in lines:
        ...
        # Рядок поза главою
        if not in_chapter:
            logging.debug(f"\nРядок поза главою ігнорується: {line}")
            continue
...
def process_init_project(input_file: str):
    ...
    logging.info(f"\nІніціалізація проекту у {project_root}")
...
if __name__ == '__main__':
    if not Path(INPUT_TEXT_FILE).exists():
        print(f"\nВхідний файл не знайдено: {INPUT_TEXT_FILE}")
        sys.exit(1)
        
    ensure_folder(OUTPUT_FOLDER)
    print(f"\nTTS_MODE={TTS_MODE}, SOUNDS_MODE={SOUNDS_MODE}\n ")
    try:
