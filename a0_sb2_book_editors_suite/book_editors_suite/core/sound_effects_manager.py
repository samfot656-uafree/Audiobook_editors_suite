...
#=======================    
    def copy_sound_effects_files(self, target_folder: str) -> Dict[str, bool]:
        """
        Копіює файли звукових ефектів з sound_effects_list.json до target_folder.
        
        Args:
            target_folder (str): Шлях до папки input_sounds_effects
            
        Returns:
            Dict[str, bool]: Словник з результатами копіювання для кожного тегу
        """
        sound_effects = self.read_sound_effects_list()
        results = {}
        ...
        for i in range(1, 100):  # S01 до S99
            tag = f"S{i:02d}"  # Форматуємо з ведучим нулем
            ...
            if tag in sound_effects:
                ...
                if file_path and os.path.exists(file_path):
                    try:
                        ...
                        # Нове ім'я файлу: тег + оригінальне розширення
                        new_filename = f"{tag}{extension}"
                        ...
                        if self.logger:
                            self.logger.info(f"Скопійовано файл для {tag}: \n{file_path} -> \n{target_path}")
                            
                    except Exception as e:
                        results[tag] = False
                        if self.logger:
                            self.logger.error(f"Помилка копіювання файлу для {tag}: \n{e}")
                else:
                    results[tag] = False
                    if self.logger:
                        if not file_path:
                            self.logger.warning(f"Для тегу {tag} не вказано file_path")
                        else:
                            self.logger.warning(f"Файл не знайдено для тегу {tag}: \n{file_path}")
            else:
                results[tag] = False
        ...
        if self.logger:
            self.logger.info(f"Копіювання звукових ефектів завершено: \n{successful}/{len(results)} успішно")
