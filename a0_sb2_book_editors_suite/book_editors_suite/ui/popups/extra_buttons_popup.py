...
#        self.title = f"{self.editor_name} \nДодаткові кнопки"
    ...
    def _build_interface(self):
        ...
        self.btn_copy_sound_eff_files = Button(text="Копіювати ефекти", font_size=bbtn_font_size)
        ...
        if self.editor_name == "accent_editor":
            ...
            for btn in (self.btn_theme, self.btn_bookmark_start, self.btn_save_txt, self.btn_copy_sound_eff_files, self.btn_back):
                btn_grid.add_widget(btn)
            self.extra_button_list = (self.btn_theme, self.btn_bookmark_start, self.btn_save_txt,  self.btn_copy_sound_eff_files, self.btn_back)
        ...
        if self.editor_name == "sound_effects_editor":
            self.btn_copy_sound_eff_files.bind(on_press=self.copy_sound_effects_files)
...
#=========
    def copy_sound_effects_files(self, *_):
        """Копіює файли звукових ефектів в папку проекту зі стандартними назвами."""
        try:
            self.main_app.copy_sound_effects_files()
            #self.dismiss()
        except Exception as e:
            self.main_app.base_editor.logger.error(f"Помилка збереження mp3: {e}")
            self.show_error_popup(f"Не вдалося зберегти ефекти. \nМетод не реалізовано:\n{e}")
#=======
