...
# додати метод перенесення файлів ефектів до INPUT_SOUNDS_EFFECTS_FOLDER
...
class SoundEffectsEditor(App):
    ...
    def __init__(self, book_project_name: str, input_text_file: str = None, **kwargs):
        ...
        self.config = self.base_editor.config_manager.load_for_editor('sound_effects_editor')
        ...
        self.input_sounds_effects_folder = self.base_editor.config.get('INPUT_SOUNDS_EFFECTS_FOLDER', '')
...
#============================
    ...
    def copy_sound_effects_files(self, instance=None):
        """Копіює файли звукових ефектів до папки input_sounds_effects"""
        try:
            # Отримуємо шлях до папки з конфігурації
            ...
            target_folder = f"{project_path}/inputs/input_sounds_effects"
 

            #====
                     
            #target_folder = self.config.get("INPUT_SOUNDS_EFFECTS_FOLDER","")
            mess = f"target_folder: \n{target_folder}"
            self.show_popup("target_folder ", mess)
  
            
            if not target_folder:
                self.show_error_popup("Не вказано папку для звукових ефектів в конфігурації")
                return
                
            # Викликаємо метод копіювання
            results = self.sound_effects_manager.copy_sound_effects_files(target_folder)
            ...
            self.show_popup(
                ...
                f"Скопійовано файлів: \n{successful}/{total}\n\n"
                f"Папка призначення:\n{target_folder}"
            )
            
            self.base_editor.logger.info(f"Копіювання звукових ефектів: \n{successful}/{total} успішно")
            
        except Exception as e:
            error_msg = f"Помилка при копіюванні звукових ефектів: \n{str(e)}"
...
#============================
    ...
    def save_file(self):
        ...
        try:
            input_file = self.base_editor.config.get('INPUT_TEXT_FILE', '')
            if not input_file:
                self.show_error_popup("Вхідний текстовий файл не вказаний у конфігурації")
                return
            ...
            if success:
                ...
                self.base_editor.logger.info(f"Файл успішно збережено: \n{input_file}")
            else:
